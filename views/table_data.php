...
<h1><?= e($db) ?>.<?= e($table) ?></h1>
<p>
    <a href="<?= e(url('/table.php?db=' . $dbSeg . '&table=' . $tableSeg . '&view=structure')) ?>">View structure</a>
    | <a href="<?= e(url('/table.php?db=' . $dbSeg . '&table=' . $tableSeg . '&view=export_csv')) ?>">Export CSV</a>
    | <a href="<?= e(url('/table.php?db=' . $dbSeg . '&table=' . $tableSeg . '&view=print')) ?>" target="_blank">Print / Save as PDF</a>
    <?php if (in_array($user['role'], [ROLE_EDITOR, ROLE_ADMIN], true)): ?>
        | <a href="<?= e(url('/table.php?db=' . $dbSeg . '&table=' . $tableSeg . '&view=new')) ?>">Insert row</a>
    <?php endif; ?>
</p>
...
<?php if ($primaryKey !== null && in_array($user['role'], [ROLE_EDITOR, ROLE_ADMIN], true)): $pkSeg = rawurlencode((string) $row[$primaryKey]); ?>
    <a href="<?= e(url('/table.php?db=' . $dbSeg . '&table=' . $tableSeg . '&view=edit&pk=' . $pkSeg)) ?>">Edit</a>
    <form method="post" action="<?= e(url('/table.php')) ?>" class="inline-form" data-confirm="Delete this row?">
        <input type="hidden" name="db" value="<?= e($db) ?>">
        <input type="hidden" name="table" value="<?= e($table) ?>">
